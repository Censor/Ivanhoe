...
void MaterialValueExplain (int c)
{
  ...
  va3 = (sint16) va3;
  va4 = (sint16) va4;
  EE ("values: %d:%d:%d:%d phase %d -> ", va1, va2, va3, va4, phase);
  if (phase < 8)
    {
      va4 *= 8 - phase;
      va3 *= phase;
  ...
  Value *= wt;
  Value /= 10;
  EE ("weights %d:%d -> Value: %d\n", wWEIGHT, bWEIGHT, Value);
  tok = InitTokens (wP, wN, wB, wBL, wBD, wR, wQ, bP, bN, bB, bBL, bBD, bR, bQ);
  fl = InitFlags (wP, wN, wB, wBL, wBD, wR, wQ, bP, bN, bB, bBL, bBD, bR, bQ);
  EE ("Token: %d Flags: %x\n", tok, fl);
}
#endif
